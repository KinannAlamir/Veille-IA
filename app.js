...
function renderPreferencesList() {
    ...
    state.selectedTopics.forEach((topicId, index) => {
        ...
        div.innerHTML = `
            ...
            <!-- Second row: Quota Indicator -->
            <div class="flex flex-wrap items-center gap-1.5 pt-2 border-t border-slate-200/50 mt-1 select-none">
                ${quotaSelectorHtml}
                ${visualPriorityHtml}
            </div>
        `;
                ...
                <div class="border border-slate-200 rounded-xl overflow-hidden bg-slate-50/50 hover:bg-slate-50 transition-colors cursor-pointer" onclick="shareDirectly('${news.id}')">
                    ...
                    <div class="p-4 space-y-3">
                        ...
                        <div class="pt-2 border-t border-slate-100/65">
                            <span class="block text-[9px] uppercase font-bold text-brand-violet tracking-wider mb-2 select-none font-sans">Points clés</span>
...
function generateDraftText(isLoadDefault = false) {
    ...
    if (format === "linkedin") {
        ...
        badgeEl.textContent = `Post LinkedIn • Personal Branding (C-Suite)`;
        ...
        titleText = `Lettre Mensuelle CTO Advisory - Décideurs`;
        ...
        titleText = `Note Opérationnelle — Cadrage Stratégique`;
